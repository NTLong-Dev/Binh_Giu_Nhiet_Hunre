...
window.onload = function () {
	...
	var tags = ["Bình Nhựa ", "Bình Tyso", "Cốc Cafe"];
...
nameProduct = decodeURIComponent(nameProduct);
    ...
    var s = addThongSo('Thương hiệu', sanPhamHienTai.detail.screen);
    s += addThongSo('Loại bảo hành', sanPhamHienTai.detail.os);
    s += addThongSo('Xuất xứ', sanPhamHienTai.detail.camara);
    s += addThongSo('Loại bình nước', sanPhamHienTai.detail.camaraFront);
    s += addThongSo('Kích Thước', sanPhamHienTai.detail.cpu);
    s += addThongSo('Dung tích', sanPhamHienTai.detail.ram);
    s += addThongSo('Khả năng giữ nhiệt', sanPhamHienTai.detail.rom);
    s += addThongSo('Chất Liệu', sanPhamHienTai.detail.microUSB);
    info.innerHTML = s;
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        ...
        default:
            var alldetail = '<select style="width: 150px; padding: 7px; "><option value="">Hồng</option><option value="">Đen</option><option value="">Xanh</option></select>' ;
            var values = ' <div class="quantity"><button class="minus" onclick="minus1()">-</button><input type="number" class="count" value="1"><button class="plus" onclick="plus1()">+</button></div>'
            return `<br>Màu Săc<br><br>` + alldetail +`<br><br>Số Lượng<br><br>` + values;
    }
}
...
function suggestion(){
    ...
    // if(sanPhamTuongTu.length) {
        ...
        addKhungSanPham(sanPhamTuongTu, 'Bạn có thể thích', ['#008000',''], div);
    // }
}